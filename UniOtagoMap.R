...
nz <- map_data("nz")
map('nz')
map<-map('nz', xlim = c(166, 175), ylim = c(-48, -40))
map+
...
gg2 +
  geom_point(data = labs, aes(x = long, y = lat), color = "red", size = 4)+labs(x="Longitude", y="Latitude")+theme(axis.text=element_text(size=10), axis.title=element_text(size=18,face="bold"))+ theme_nothing()